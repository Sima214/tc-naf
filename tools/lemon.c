...
PRIVATE char *msort(char *, char **, int (*)(const char *, const char *));
...
PRIVATE int resolve_conflict(struct action *, struct action *);
...
*/
PRIVATE void handle_D_option(char *z) {
  size_t pazl;
  char **paz;
  nDefine++;
...
PRIVATE void handle_d_option(char *z) {
  size_t outputDirLen = strlen(z) + 1;
  outputDir = malloc(outputDirLen);
  if(outputDir == 0) {
    fprintf(stderr, "out of memory\n");
    exit(1);
  }
...
PRIVATE void handle_T_option(char *z) {
  size_t user_templatename_len = strlen(z) + 1;
  user_templatename = malloc(user_templatename_len);
...
/* forward reference */
PRIVATE const char *minimum_size_type(int lwr, int upr, int *pnByte);

/* Print a single line of the "Parser Stats" output
*/
PRIVATE void stats_line(const char *zLabel, int iValue) {
...
** of the n-th field.
*/
PRIVATE void errline(int n, int k, FILE *err) {
  int spcnt, i;
  if(g_argv[0])
...
*/
PRIVATE int handleflags(int i, FILE *err) {
  int v;
  int errcnt = 0;
  int j;
...
*/
PRIVATE int handleswitch(int i, FILE *err) {
  int lv = 0;
  double dv = 0.0;
  char *sv = 0, *end;
  char *cp;
...
PRIVATE void parseonetoken(struct pstate *psp) {
  const char *x;
...
to follow the previous rule.");
    ...
    case WAITING_FOR_DECL_ARG:
      if(x[0] == '{' || x[0] == '\"' || ISALNUM(x[0])) {
        ...
        int addLineMacro;
        static const size_t zLineLen = 50;
        char zLine[zLineLen];
        zNew = x;
        if(zNew[0] == '"' || zNew[0] == '{')
          zNew++;
        ...
        if(addLineMacro) {
          ...
          snprintf(zLine, zLineLen, "#line %d ", psp->tokenlineno);
          nLine = lemonStrlen(zLine);
          n += nLine + lemonStrlen(psp->filename) + nBack;
        }
        *psp->declargslot = realloc(*psp->declargslot, n);
...
*/
PRIVATE void preprocess_input(char *z) {
  int i, j, k, n;
  int exclude = 0;
  int start = 0;
  int lineno = 1;
...
PRIVATE char *file_makename(struct lemon *lemp, const char *suffix) {
  ...
  name[0] = 0;
  if(outputDir) {
    strncpy(name, outputDir, sz);
    strncat(name, "/", sz);
  }
  strncat(name, filename, sz);
  cp = strrchr(name, '.');
  if(cp)
    *cp = 0;
  strncat(name, suffix, sz);
  return name;
}
...
void ReportOutput(struct lemon *lemp) {
  ...
  for(i = 0; i < lemp->nxstate; i++) {
    ...
    if(lemp->basisflag)
      ...
      cfp = stp->cfp;
    while(cfp) {
      static const size_t buf_len = 20;
      char buf[buf_len];
      if(cfp->dot == cfp->rp->nrhs) {
        snprintf(buf, buf_len, "(%d)", cfp->rp->iRule);
        fprintf(fp, "    %5s ", buf);
      } else {
        fprintf(fp, "          ");
      }
...
PRIVATE char *pathsearch(char *argv0, char *name, int modemask) {
  ...
  if(cp) {
    c = *cp;
    *cp = 0;
    size_t path_len = strlen(argv0) + strlen(name) + 2;
    path = malloc(path_len);
    if(path)
      snprintf(path, path_len, "%s/%s", argv0, name);
    *cp = c;
  } else {
    pathlist = getenv("PATH");
    if(pathlist == 0)
      pathlist = ".:/bin:/usr/bin";
    size_t pathbufl = strlen(pathlist) + 1;
    pathbuf = malloc(pathbufl);
    size_t path_len = strlen(pathlist) + strlen(name) + 2;
    path = malloc(path_len);
    if((pathbuf != 0) && (path != 0)) {
      pathbufptr = pathbuf;
      strncpy(pathbuf, pathlist, pathbufl);
      while(*pathbuf) {
        cp = strchr(pathbuf, ':');
        if(cp == 0)
        ...
        c = *cp;
        *cp = 0;
        snprintf(path, path_len, "%s/%s", pathbuf, name);
        *cp = c;
        if(c == 0)
          pathbuf[0] = 0;
        else
...
PRIVATE FILE *tplt_open(struct lemon *lemp) {
  ...
  static const size_t buf_len = 1000;
  char buf[buf_len];
  FILE *in;
  char *tpltname;
  char *cp;
  ...
  if(cp) {
    snprintf(buf, buf_len, "%.*s.lt", (int)(cp - lemp->filename), lemp->filename);
  } else {
    snprintf(buf, buf_len, "%s.lt", lemp->filename);
  }
  if(access(buf, 004) == 0) {
    tpltname = buf;
...
PRIVATE int translate_code(struct lemon *lemp, struct rule *rp) {
  ...
  if(rp->nrhs == 0) {
    ...
    ** direct writing is allowed */
    ...
    snprintf(zOvwrt, 900, "/*%s-overwrites-%s*/",
...
void print_stack_union(
  ...
  */
  for(i = 0; i < lemp->nsymbol; i++) {
    ...
    if(types[hash] == 0) {
      sp->dtnum = hash + 1;
      types[hash] = malloc(lemonStrlen(stddt) + 1);
      if(types[hash] == 0) {
        fprintf(stderr, "Out of memory.\n");
        exit(1);
      }
...
*/
PRIVATE int axset_compare(const void *a, const void *b) {
...
*/
PRIVATE void writeRuleText(FILE *out, struct rule *rp) {
  int j;
  fprintf(out, "%s ::=", rp->lhs->name);
  for(j = 0; j < rp->nrhs; j++) {
...
** token actions.
*/
PRIVATE int stateResortCompare(const void *a, const void *b) {
...
void Strsafe_init(void) {
  ...
  if(x1a) {
    x1a->size = 1024;
    x1a->count = 0;
    x1a->tbl = calloc(1024, sizeof(x1node) + sizeof(x1node *));
    if(x1a->tbl == 0) {
      free(x1a);
      x1a = 0;
    } else {
...
int Strsafe_insert(const char *data) {
  ...
  if(x1a->count >= x1a->size) {
    ...
    array.tbl = calloc(arrSize, sizeof(x1node) + sizeof(x1node *));
    if(array.tbl == 0)
...
void Symbol_init(void) {
  ...
  if(x2a) {
    x2a->size = 128;
    x2a->count = 0;
    x2a->tbl = calloc(128, sizeof(x2node) + sizeof(x2node *));
    if(x2a->tbl == 0) {
      free(x2a);
      x2a = 0;
    } else {
...
int Symbol_insert(struct symbol *data, const char *key) {
  ...
  if(x2a->count >= x2a->size) {
    ...
    array.tbl = calloc(arrSize, sizeof(x2node) + sizeof(x2node *));
    if(array.tbl == 0)
      return 0; /* Fail due to malloc failure */
...
void State_init(void) {
  ...
  if(x3a) {
    x3a->size = 128;
    x3a->count = 0;
    x3a->tbl = calloc(128, sizeof(x3node) + sizeof(x3node *));
    if(x3a->tbl == 0) {
      free(x3a);
      x3a = 0;
    } else {
...
int State_insert(struct state *data, struct config *key) {
  ...
  if(x3a->count >= x3a->size) {
    ...
    array.tbl = calloc(arrSize, sizeof(x3node) + sizeof(x3node *));
    if(array.tbl == 0)
      return 0; /* Fail due to malloc failure */
...
void Configtable_init(void) {
  ...
  if(x4a) {
    x4a->size = 64;
    x4a->count = 0;
    x4a->tbl = calloc(64, sizeof(x4node) + sizeof(x4node *));
    if(x4a->tbl == 0) {
      free(x4a);
      x4a = 0;
    } else {
...
int Configtable_insert(struct config *data) {
  ...
  if(x4a->count >= x4a->size) {
    ...
    array.tbl = calloc(arrSize, sizeof(x4node) + sizeof(x4node *));
    if(array.tbl == 0)
      return 0; /* Fail due to malloc failure */
